...
func (t *TreeSitterUtil) IsCodeSyntax(code string) bool {
	...
	if tree == nil {
		t.logger.Error("解析代码失败", zap.String("language", t.language))
		return false
	}
	defer tree.Close()
...
// InterceptSyntaxErrorCode 拦截语法错误代码
func (t *TreeSitterUtil) InterceptSyntaxErrorCode(choicesText, prefix, suffix string, timeStart int64, timeOutThreshold int64) string {
	start := time.Now()
	defer t.calculateTime("InterceptSyntaxErrorCode", start)
	...
	for i := 0; i < maxCutCount; i++ {
		...
		if t.IsCodeSyntax(prefix+cutCode+suffix) && strings.TrimSpace(cutCode) != "" {
			if i != 0 {
				t.logger.Info(fmt.Sprintf("%s切割代码成功，切割前的内容：%s， 切割后的内容：%s", t.language, choicesText, cutCode))
			}
		...
		// 检查超时
		if (time.Now().UnixMilli() - timeStart) > timeOutThreshold {
			t.logger.Info(fmt.Sprintf("%s切割代码超时: %s", t.language, choicesText))
			return choicesText
		}
	}
	
	t.logger.Info(fmt.Sprintf("%s切割代码失败: %s", t.language, choicesText))
	return choicesText
}
