...
/**
 * Initialize company
 * @param company Pointer to company to initialize
 */
...
int addEmployee(Company* company, const char* name, const char* department, 
    ...
    for (int i = 0; i < 7; i++) {
        ...
        // Default work hours (8 hours on weekdays, 0 on weekends)
        if (i < 5) {
            emp->weeklySchedule[i].hoursWorked = 8.0f;
        } else {
            emp->weeklySchedule[i].hoursWorked = 0.0f;
        }
    }
...
float calculateMonthlySalary(Employee* emp) {
    ...
    float monthlyHours = weeklyHours * 4.33f;
    return monthlyHours * emp->hourlyWage;
}
 ...
 * @return Number of employees found
 */
 ...
 * @param emp Pointer to employee
 * @param currentDate Current date for calculating service years
 */
void printEmployee(const Employee* emp, const Date* currentDate) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (emp->weeklySchedule[i].isHoliday) {
            printf(" (Holiday)");
        }
        printf("\n");
    }
