...
/**
 * Convert seat class enum to string
 * @param seatClass Seat class enum
...
/**
 * Initialize airline system
 * @param system Pointer to system to initialize
 */
...
/**
 * Add new flight
 * @param system Pointer to airline system
 ...
 * @param totalSeats Total seats in economy class
 * @param businessSeats Seats in business class
 * @param firstClassSeats Seats in first class
 * @param economyPrice Base economy price
 * @param businessPrice Business class price
 * @param firstClassPrice First class price
 * @param isInternational Whether it's an international flight
 * @return 1 if successful, 0 if flight list is full
 */
int addFlight(AirlineSystem* system, const char* flightNumber, const char* airline,
              ...
              bool isInternational) {
    if (system->flightCount >= MAX_FLIGHTS) {
        return 0;
    }
 ...
 * @param system Pointer to airline system
 * @param flightNumber Flight number to find
 * @return Pointer to flight if found, NULL otherwise
 */
 ...
 * @return Booking ID if successful, -1 if flight not found or no seats available
 */
int bookFlight(AirlineSystem* system, const char* flightNumber, const char* passengerName,
    ...
    if (flight == NULL || flight->availableSeats[seatClass] <= 0) {
        return -1;
    }
    
    if (system->bookingCount >= MAX_PASSENGERS) {
        return -1;
    }
 ...
 * @return 1 if successful, 0 if booking not found
 */
 ...
 * @param flight Pointer to flight
 ...
 * @param system Pointer to airline system
 * @return Airport code of most popular destination, NULL if no flights
 */
const char* findMostPopularDestination(AirlineSystem* system) {
    if (system->flightCount == 0) {
        return NULL;
    }
    
    // Count destinations
    int counts[20] = {0}; // Maximum different destinations
